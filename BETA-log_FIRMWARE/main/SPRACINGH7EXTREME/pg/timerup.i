...
extern timerUpConfig_t timerUpConfig_SystemArray[17]; extern timerUpConfig_t timerUpConfig_CopyArray[17]; static inline const timerUpConfig_t* timerUpConfig(int _index) { return &timerUpConfig_SystemArray[_index]; } static inline timerUpConfig_t* timerUpConfigMutable(int _index) { return &timerUpConfig_SystemArray[_index]; } static inline timerUpConfig_t (* timerUpConfig_array(void))[17] { return &timerUpConfig_SystemArray; } struct _dummy;
# 31 "./src/main/pg/timerup.c" 2

extern void pgResetFn_timerUpConfig(timerUpConfig_t *); timerUpConfig_t timerUpConfig_SystemArray[17]; timerUpConfig_t timerUpConfig_CopyArray[17]; extern const pgRegistry_t timerUpConfig_Registry; const pgRegistry_t timerUpConfig_Registry __attribute__ ((section(".pg_registry"), used, aligned(4))) = { .pgn = 549 | (0 << 12), .length = 17, .size = (sizeof(timerUpConfig_t) * 17) | PGR_SIZE_SYSTEM_FLAG, .address = (uint8_t*)&timerUpConfig_SystemArray, .copy = (uint8_t*)&timerUpConfig_CopyArray, .ptr = 0, .reset = {.fn = (pgResetFunc*)&pgResetFn_timerUpConfig}, };

void pgResetFn_timerUpConfig(timerUpConfig_t *config)
{
    for (unsigned timno = 0; timno < 17; timno++) {
        config[timno].dmaopt = (-1);
    }
