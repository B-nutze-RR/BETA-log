...
static int write_word(config_streamer_t *c, config_streamer_buffer_align_type_t *buffer)
{
    ...
    uint32_t flashOverflowAddress = ((flashPartition->endSector + 1) * flashGeometry->sectorSize);

    uint32_t flashAddress = flashStartAddress + dataOffset;
    if (flashAddress + 8 > flashOverflowAddress) {
        return -3;
    }
...
# 393 "./src/main/config/config_streamer.c" 3 4
   _Bool 
# 393 "./src/main/config/config_streamer.c"
        onPageBoundary = (flashAddress % flashPageSize == 0);
    if (onPageBoundary) {

        
# 396 "./src/main/config/config_streamer.c" 3 4
       _Bool 
# 396 "./src/main/config/config_streamer.c"
            firstPage = (flashAddress == flashStartAddress);
        if (!firstPage) {
    ...
    flashPageProgramContinue((uint8_t *)buffer, 8);
# 519 "./src/main/config/config_streamer.c"
    c->address += 8;
    return 0;
}
