...
static void rescueAttainPosition()
{
    ...
    const float ct = cos(((gpsRescueAngle[AI_PITCH] / 10) / 10.0f * 0.0174532925f));
    ...
    {if (debugMode == (DEBUG_GPS_RESCUE_THROTTLE_PID)) {debug[(0)] = (throttle.Kp * zVelocityError);}};
    {if (debugMode == (DEBUG_GPS_RESCUE_THROTTLE_PID)) {debug[(1)] = (throttle.Ki * zVelocityIntegral);}};
    {if (debugMode == (DEBUG_GPS_RESCUE_THROTTLE_PID)) {debug[(2)] = (throttle.Kd * zVelocityDerivative);}};
    {if (debugMode == (DEBUG_GPS_RESCUE_THROTTLE_PID)) {debug[(3)] = (rescueState.sensor.zVelocity);}};
}
