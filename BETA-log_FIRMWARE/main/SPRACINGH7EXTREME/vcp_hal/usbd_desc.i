...
# 76 "./src/main/vcp_hal/usbd_desc.c"
...
uint8_t *USBD_VCP_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  {
    USBD_GetString((uint8_t *)"STM32 Virtual ComPort in HS Mode", USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)"STM32 Virtual ComPort in FS Mode", USBD_StrDesc, length);
  }
  return USBD_StrDesc;
}
...
static void Get_SerialNum(void)
{
  ...
  deviceserial0 = (*(uint32_t*)(0x1FF1E800UL));
  deviceserial1 = (*(uint32_t*)((0x1FF1E800UL) + 4));
  deviceserial2 = (*(uint32_t*)((0x1FF1E800UL) + 8));
...
# 334 "./src/main/vcp_hal/usbd_desc.c"
static void IntToUnicode (uint32_t value , uint8_t *pbuf , uint8_t len)
{
  uint8_t idx = 0;
