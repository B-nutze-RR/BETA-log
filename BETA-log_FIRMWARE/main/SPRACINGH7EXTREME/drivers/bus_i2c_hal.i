...
# 112 "./src/main/drivers/bus_i2c_hal.c"
    ...
    if (reg_ == 0xFF)
        status = HAL_I2C_Master_Transmit(pHandle ,addr_ << 1, &data, 1, ((uint32_t)0x1000));
    else
        status = HAL_I2C_Mem_Write(pHandle ,addr_ << 1, reg_, (0x00000001U), &data, 1, ((uint32_t)0x1000));
...
# 167 "./src/main/drivers/bus_i2c_hal.c"
    ...
    if (reg_ == 0xFF)
        status = HAL_I2C_Master_Receive(pHandle ,addr_ << 1, buf, len, ((uint32_t)0x1000));
    else
        status = HAL_I2C_Mem_Read(pHandle, addr_ << 1, reg_, (0x00000001U),buf, len, ((uint32_t)0x1000));
